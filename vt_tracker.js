...
if (!window._vtTrackerLoaded) {
    ...
    function _vtUpdateBadge(data, videoId) {
        ...
        const icons = [];
        if (rating === 'like') icons.push(SVG_LIKE);
        if (rating === 'dislike') icons.push(SVG_DISLIKE);
        ...
        if (data.badgeEl) {
            data.badgeEl.innerHTML = icons.join('');
            data.badgeEl.style.display = 'flex';
            data.badgeEl.style.alignItems = 'center';
            data.badgeEl.style.gap = '2px';
        }
    }
