...
function _countInFolder(folderId) {
    ...
    return _allBookmarks.filter(b => allSubIds.has(b.folderId)).length;
}
...
function _makeFolderNode({ id, name, count, depth, hasKids, isSpecial }) {
    ...
    item.onclick = () => {
        _activeFolderId = isSpecial ? id : id;
...
function _getFilteredBookmarks() {
    ...
    if (_activeFolderId === '__all__') {
        ...
        const allSub = _getAllSubfolderIds(_activeFolderId);
        allSub.add(_activeFolderId);
        items = items.filter(b => allSub.has(b.folderId));
    }
...
function _makeCard(bm) {
    ...
    openBtn.className = 'bv-card-act';
    openBtn.textContent = '🔗';
    openBtn.title = '開啟連結';
    openBtn.onclick = (e) => { e.stopPropagation(); chrome.tabs.create({ url: bm.url }); };

    const copyBtn = document.createElement('button');
    copyBtn.className = 'bv-card-act';
    copyBtn.textContent = '📋';
    copyBtn.title = '複製網址';
    copyBtn.onclick = (e) => {
        e.stopPropagation();
